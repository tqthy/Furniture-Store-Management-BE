...
class ProductVariantController {
    createProductVariant = async(req, res) => {
        ...
        const { sku, price, color, size, buyingPrice } = req.body;
        if (!productId || !sku || !price || !color || !size || !buyingPrice) {
        ...
        try {
            const response = await ProductVariantService.createProductVariant(productId, sku, price, color, size, buyingPrice);
    ...
    updateProductVariant = async(req, res) => {
        const { id } = req.params;
        const { sku, price, color, size, buyingPrice } = req.body;
        if (!id || !sku || !price || !color || !size || !buyingPrice) {
            return res.status(200).json({
                EM: 'Missing required fields',
                EC: 1,
                DT: ''
        ...
        try {
            const response = await ProductVariantService.updateProductVariant(id, sku, price, color, size, buyingPrice);
            return res.status(200).json(response);
        } catch(error) {
            return res.status(500).json({ error: error.message });
        }
